...
export async function treatedPng(blob,layer,color,check=()=>{}){
  ...
  try{
    ...
    if(layer.mode!=='original'||hasPrintTexture(layer))for(let y=0;y<height;y+=128){
      check();const pixels=ctx.getImageData(0,y,width,Math.min(128,height-y));treatPixels(pixels.data,layer,color);applyPrintTexture(pixels.data,width,pixels.height,layer,{fullWidth:width,fullHeight:height,offsetY:y});ctx.putImageData(pixels,0,y);await turn();
    }
