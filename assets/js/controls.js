...
export function installSliderControls(defaultArtworkValue){
  ...
  for(const range of document.querySelectorAll('input[type="range"]')){
    ...
    if(!number.hasAttribute('data-art-num')&&!number.id.match(/^(inertia|grid)/)){
      number.addEventListener('input',()=>{if(number.value===''||!Number.isFinite(number.valueAsNumber))return;range.value=number.value;range.dispatchEvent(new Event('input',{bubbles:true}));number.value=range.value;});
    ...
    if(!reset){
      ...
      reset.addEventListener('click',()=>{range.value=reset.dataset.defaultOverride??getDefault();range.dispatchEvent(new Event('input',{bubbles:true}));range.dispatchEvent(new Event('change',{bubbles:true}));});
    }
  ...
  document.addEventListener('wheel',event=>{
    ...
    range.value=String(Number((Number(range.value)+direction*step*(event.shiftKey?10:1)).toFixed(6)));
