...
import {psdSupported} from './print-package.js?v=86';
export function installPrintLayoutUI({toggle,container,status}){
  ...
  function set(next,garment){surfaces=next;key=garment;ready=true;container.replaceChildren();
    ...
    for(const surface of surfaces){
      ...
      const details=el('details','export-closeups print-layout'),summary=el('summary'),title=el('span'),info=el('p','muted'),fields=el('div','export-options');
      summary.append(title);details.append(summary);const include=el('label','check-option'),checkbox=el('input');checkbox.type='checkbox';checkbox.checked=size.enabled;checkbox.addEventListener('change',()=>{size.enabled=checkbox.checked;update();});include.append(checkbox,document.createTextNode('Include '+surface.name));details.append(include);
      ...
      function update(){try{const plan=printPlan(surface,size);title.textContent=`${surface.name} · ${plan.inches.artWidth.toFixed(2)} × ${plan.inches.artHeight.toFixed(2)} in${size.enabled?'':' · off'}`;info.textContent=`${plan.width} × ${plan.height} px canvas. Artwork height follows its proportions.`;info.removeAttribute('role');}catch(e){title.textContent=surface.name+(size.enabled?'':' · off');info.textContent=e.message;info.setAttribute('role','status');}}
      details.append(fields,fit,info);container.append(details);update();
    }sync();
  }
