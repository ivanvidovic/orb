import {patchSVG} from './patch-shapes.js?v=92';
import {decodeArtworkImage} from './artwork-decode.js?v=92';
import {cleanFilename} from './design-format.js?v=92';
...
export async function addPrintLayouts(zip,data,plans,colorFor,{check=()=>{},message=()=>{},advance=()=>{}}={}){
  ...
  for(const [index,plan] of plans.entries()){
    check();const worker=new Worker(new URL('./print-psd-worker.js?v=92',import.meta.url),{type:'module'}),layers=[];
    try{
      ...
      for(const [i,desc] of plan.layers.entries()){
        ...
        if(desc.patch){const p=desc.patch,m=p.matrix,mm=25.4/plan.ppi,width=Math.hypot(m[0],m[1])*mm,height=Math.hypot(m[2],m[3])*mm,path=`Patches/${String(index+1).padStart(2,'0')}_${cleanFilename(plan.name)}/${layer.id}_${cleanFilename(layer.name)}`;zip.file(path+'_Cut-outline.svg',patchSVG(p.contours,width,height));zip.file(path+'_Dimensions.json',JSON.stringify({units:'mm',width,height,thickness:p.thickness,stitch:p.stitch,leatherColor:p.color,placement:layer.slot,engravingLayer:layer.name,layoutCanvasInches:plan.inches,outlineMatrixPixels:m},null,2));}
        layers.push({patch:desc.patch||undefined,id:layer.id,name:layer.name,placement:layer.slot,visible:layer.visible!==false,assetId:layer.assetId,mode:layer.mode,printPattern:layer.printPattern||'none',glow:!!layer.glow,uvReactive:!!layer.uvReactive,emission:layer.emission,matrixPixels:desc.matrix,sourcePixels:response.sourcePixels,rasterScale:response.rasterScale});exported.add(layer.id);advance();
      }
  ...
  if(plans.some(p=>p.layers.some(l=>l.patch)))zip.file('Patches/README.txt','Each SVG is the leather cut boundary at the dimensions recorded beside it. The layered PSD contains black engraving artwork clipped to that boundary; leather grain, edge thickness, stitches and simulated lighting are preview only. Keep the cut path separate from the engraving job. Artwork scale in the export dialog also scales patch width and height. Thickness stays in millimetres. Confirm the blank, edge clearance and laser/cutting settings with your fabricator. Interior contours are cutouts.');
