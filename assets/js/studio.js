...
function migrateCameraFrameV3(saved){
  ...
  if(saved.framing==='proportions-v2'){
    ...
    const full=!saved.view||['front','angle','side','right','backangle','back'].includes(saved.view);
    if(activeGarmentId==='womens-hoodie'&&full&&focus.distanceTo(center)<.03)return {...saved,r:saved.r/.9,focus:focus.sub(center).multiplyScalar(1/.9).add(center).toArray(),framing:'proportions-v3'};
    return {...saved,framing:'proportions-v3'};
  }
  const previous=saved.framing==='proportions-v1'?{center:torsoFrame(activeGarmentId).center,distance:torsoDistance(activeGarmentId)*(activeGarmentId==='womens-hoodie'?.9:1)}:saved.framing==='torso-v1'?previousTorsoFrame(activeGarmentId):null;
  ...
  return {...saved,r:saved.r*ratio,focus:focus.sub(oldCenter).multiplyScalar(ratio).add(garmentCenter).toArray(),framing:'proportions-v3'};
}
...
function syncInkUi(){
  ...
  for(const [id,fallback] of [['printPattern','none'],['printSize',40],['printAngle',45],['printMarkSize',50],['printTone',100],['printErosion',0],['printPixelScale',35],['printDensity',50],['printRounding',0],['printBranchMode','repeat']]){
  ...
  const organic=['maze','branching'].includes(entry?.printPattern);
  document.getElementById('printDensity').parentElement.hidden=!organic;
  document.getElementById('printBranchMode').parentElement.hidden=entry?.printPattern!=='branching';
  document.getElementById('printVariation').hidden=!organic;
  document.getElementById('printVariation').disabled=disabled;
  document.querySelector('label[for=printSize]').textContent=organic?'Scale':'Spacing';
  document.querySelector('label[for=printMarkSize]').textContent=organic?'Thickness':entry?.printPattern==='lines'?'Line width':entry?.printPattern==='grain'?'Grain size':'Dot size';
...
const needsArtworkTreatment=layer=>(layer.printRounding??0)>0||layer.mode==='ink'||['dots','lines','grain','pixel','maze','branching'].includes(layer.printPattern);
...
function artworkSource(layer){
  if(needsArtworkTreatment(layer)){
    const image=treatedArtworkSource(layer);
    // A previously untreated Original/Tint layer has no worker image yet.
    // Keep its full-quality source visible during the first organic generation.
    if(!image&&(['maze','branching'].includes(layer.printPattern)||(layer.printRounding??0)>0)&&layer.mode!=='ink')
      return artworkSource({...layer,printPattern:'none',printRounding:0});
    return image;
  }
...
let layerDrag=null,layerScrollFrame=0,suppressLayerClickUntil=0;
// Only suppress the click generated by a completed drag. A new pointer gesture
// or keyboard activation must remain a normal layer click.
const layerList=document.getElementById('artLayers');
layerList.addEventListener('pointerdown',()=>{suppressLayerClickUntil=0;},true);
layerList.addEventListener('click',event=>{
  if(event.detail>0&&performance.now()<suppressLayerClickUntil){
    suppressLayerClickUntil=0;event.preventDefault();event.stopImmediatePropagation();
  }
},true);
...
document.addEventListener('pointerup',e=>{
  ...
  if(d.started){suppressLayerClickUntil=performance.now()+500;reorderArtwork(d.id,d.beforeId);}
  else selectArtwork(activeArtId===d.id?null:d.id);
  layerRows.get(d.id)?.querySelector('.art-drag').focus({preventScroll:true});
});
...
for(const id of ['printSize','printAngle','printMarkSize','printTone','printErosion','printPixelScale','printDensity','printRounding']){
  const input=document.getElementById(id);
  input.addEventListener('input',()=>{const entry=artEntry();if(!entry||artLoading)return;
    ...
    if(id!=='printRounding')entry.printVersion=2;entry[id]=Number(input.value);requestArtworkRender(entry);workspace?.notify();
  });
  for(const event of ['change','blur'])input.addEventListener(event,()=>printEditing=null);
}
document.getElementById('printBranchMode').onchange=e=>{const entry=artEntry();if(!entry||artLoading)return;recordArtUndo();entry.printBranchMode=e.target.value;entry.printVersion=2;requestArtworkRender(entry);workspace?.notify();};
document.getElementById('printVariation').onclick=()=>{
  const entry=artEntry();if(!entry||artLoading)return;recordArtUndo();
  entry.printSeed=((entry.printSeed??1)+1)>>>0;entry.printVersion=2;
  requestArtworkRender(entry);workspace?.notify();
};
